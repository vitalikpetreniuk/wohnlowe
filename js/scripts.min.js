...
$(function () {
    ...
    var calcValText;
    var calcValPost;
    var triangleInner = $('.triangle-in');
    var triangleImg = $('.triangle-img');
    ...
    $(window).on('load scroll', function () {
        ...
        if ((footerOffsetTop - windowScrollTop - windowHeight) < 0) {
            $('.callback').addClass('stuck');
        } else {
            $('.callback').removeClass('stuck');
        }
        var bodyHeight = $('.body').outerHeight();
        var bodyHeightPercent = bodyHeight / 100;
        var windowScrollTopPercent = windowScrollTop / bodyHeightPercent;
        var rotatePercent = windowScrollTopPercent * 1.8;
        triangleInner.css('right', windowScrollTopPercent + '%');
        triangleImg.css('transform', 'rotate(' + rotatePercent + 'deg)');
        $('.figure img').css('transform', 'rotate(' + windowScrollTop/30 + 'deg)');
    }).on('load resize', function () {
        bodyHeight = $('.body').outerHeight();
        if (window.matchMedia('(max-width: 767px)').matches) {
            if (num === 0) {
                $('.reviews-list, .video-list.row').addClass('owl-carousel slider-dots').owlCarousel({
                    items: 1,
                    margin: 32
                });
                num = 1;
            ...
            if (num === 1) {
                $('.reviews-list, .video-list.row').removeClass('owl-carousel slider-dots').owlCarousel('destroy');
                num = 0;
            }
        }
        $('.figure-in').css('height', bodyHeight);
    }).on('load', function () {
        if ($('.map').length) {
            var activeMapElement = $('.map-img path.active');
            var activeMapElementName = $('.map-list button.active').text();
            var activeMapElementTop = activeMapElement.offset().top;
            var activeMapElementLeft = activeMapElement.offset().left;
            mapToolTip.css({
                top: activeMapElementTop + 'px',
                left: activeMapElementLeft + 'px'
            }).text(activeMapElementName);
            mapToolTip.text(function () {
    ...
    $('.advantages .advantages-item').on('mouseenter', function () {
        $(this).addClass('open').find('.advantages-item-text').slideDown('fast');
    }).on('mouseleave', function () {
        $(this).removeClass('open').find('.advantages-item-text').slideUp('fast');
    });

    $('.reviews-item').on('mouseenter', function () {
        $(this).addClass('open').find('.reviews-item-img-big').slideDown('fast');
    }).on('mouseleave', function () {
        $(this).removeClass('open').find('.reviews-item-img-big').slideUp('fast');
    });
    ...
    $('.video-item-cover').on('click', function (ev) {
        $(this).addClass('open');
        $(this).siblings('.video-item-frame').find('iframe')[0].src += "&autoplay=1";
        ev.preventDefault();
    });

    $('.slider-nav .owl-dot').each(function () {
        $(this).text($(this).index() + 1);
    });

    $('.article-page-slider-in').owlCarousel({
        items: 1,
        nav: true,
        dots: false,
        loop: true,
    ...
    $('.articles-item-desc').each(function () {
        var articleDesc = $(this).text();
        var articleCut = articleDesc.substring(0, 159);
        $(this).text(articleCut + '...');
    });

    $('.accordion-item-title').on('click', function () {
        $(this).siblings('.accordion-item-cont').slideToggle('fast').closest('.accordion-item').toggleClass('open').siblings('.accordion-item').removeClass('open').find('.accordion-item-cont').slideUp('fast')
    });

    $('select').customSelect();

    $('.content-page ol li').each(function () {
        var liIndex = $(this).index() + 1;
        $(this).prepend('<span>' + liIndex + '</span>');
    });

    $('.slider-double').closest('.content-page-wrap').addClass('with-slider');
    ...
    sliderNav.each(function () {
        $(this).find('.owl-item:first-child').addClass('slide-current');
        $(this).find('.owl-item').on('click', function () {
            $(this).addClass('slide-current').siblings().removeClass('slide-current');
            var slideActive = $(this).attr('data-id');
            sliderMain.find('.owl-dot[data-id="' + slideActive + '"]').trigger('click');
        })
    }).on('translated.owl.carousel', function () {
        var slideActive = $(this).find('.owl-item.slide-current').attr('data-id');
        sliderMain.trigger('to.owl.carousel', [slideActive, 100]);
    });
    ...
    $('button.valuation-category-button').on('click', function () {
        $(this).closest('.calc-inside').addClass('with-calc');
        $(this).closest('.valuation-categories').siblings('.calc').addClass('open');
        var target = $(this).attr('data-target');
        $(this).closest('.valuation-categories').siblings('.calc').find('.calc-block[data-id=' + target + ']').addClass('active').siblings().removeClass('active');
        if($(this).closest('.valuation-categories').siblings('.calc').find('.calc-block.active .calc-screen.active .choose-buttons').length){
            $('.calc-forward').addClass('hidden');
        }else{
            $('.calc-forward').removeClass('hidden');
        }
    });

    $('.calc-block').each(function () {
        var screenLength = $(this).find('.calc-screen').length;
        $(this).find('.calc-progress span').css('width', 100 / screenLength + '%');
    });

    $('.calc-back').on('click', function () {
        $('.calc-nav').removeClass('last');
        if ($(this).closest('.calc-block').find('.calc-screen.active').is(':first-child')) {
            $('.calc-inside').removeClass('with-calc');
            $('.calc').removeClass('open');
        } else {
            $(this).closest('.calc-block').find('.calc-screen.active').removeClass('active').prev().addClass('active');
        }
        if ($(this).closest('.calc-block').find('.calc-screen.active').find('.choose-buttons').length) {
            $('.calc-forward').addClass('hidden');
        }else{
            $('.calc-forward').removeClass('hidden');
        }
    ...
    $('.calc-forward').on('click', function () {
        if (!$(this).closest('.calc-block').find('.calc-screen.active').is(':last-child')) {
            $(this).closest('.calc-block').find('.calc-screen.active').removeClass('active').next().addClass('active');
        }
        if ($(this).closest('.calc-block').find('.calc-screen.active').is(':last-child')) {
            $('.calc-nav').addClass('last');
        }
        if ($(this).closest('.calc-block').find('.calc-screen.active').find('.choose-buttons').length) {
            $('.calc-forward').addClass('hidden');
            console.log('hidden');
    ...
    $('.calc-nav-button').on('click', function () {
        var currentScreenLength = $(this).closest('.calc-block').find('.calc-screen').length;
        var currentScreenIndex = $(this).closest('.calc-block').find('.calc-screen.active').index() + 1;
        $(this).closest('.calc-block').find('.calc-progress span').css('width', 100 / currentScreenLength * currentScreenIndex + '%');
    });
    ...
    $('.calc-screen').each(function () {
        var calcId = $(this).attr('data-target');
        if ($(this).find('input[type=number]').length) {
            calcValNum = $(this).find('input[type=number]').val();
            $('input[data-id=' + calcId + ']').val(calcValNum);
        } else if ($(this).find('.choose-button.active span').length) {
            calcValText = $(this).find('.choose-button.active span').text();
            $('input[data-id=' + calcId + ']').val(calcValText);
        }
        $(this).find('input[type=number]').on('change', function () {
            calcValText = $(this).val();
            $('input[data-id=' + calcId + ']').val(calcValText);
        });
        $(this).find('.choose-button').on('click', function () {
            $(this).addClass('active').siblings().removeClass('active');
            calcValText = $(this).find('span').text();
            $('input[data-id=' + calcId + ']').val(calcValText);
            $(this).closest('.calc-block').find('.calc-forward').trigger('click');
        });
        $(this).find('.postcode-input input').on('change', function () {
            calcValPost = $(this).val();
            $('input[data-id=' + calcId + ']').val(calcValPost);
        });
    });
    ...
    $('.map-img path').on('click touchstart', function () {
        $(this).addClass('active').siblings().removeClass('active');
        var mapTarget = $(this).attr('data-id');
        var mapListItem = $('.map-list button[data-target=' + mapTarget + ']');
        var mapListItemFirst = $('.map-list .first[data-target=' + mapTarget + ']');
        var mapElementTop = $(this).offset().top;
        var mapElementLeft = $(this).offset().left;
        mapToolTip.css({
            top: mapElementTop + 'px',
            left: mapElementLeft + 'px'
        });
        if (mapListItem.length === 2) {
            mapListItemFirst.addClass('active').parent().siblings().find('.active').removeClass('active');
            mapListItemFirst.closest('ul').siblings().find('.active').removeClass('active');
            $('.map-block.first[data-id=' + mapTarget + ']').addClass('active').siblings().removeClass('active');
            mapToolTip.text(mapListItemFirst.text());
            mapToolTip.text(function () {
                return $(this).text().replace('_', ' ');
            });
        } else {
            mapListItem.addClass('active').parent().siblings().find('.active').removeClass('active');
            mapListItem.closest('ul').siblings().find('.active').removeClass('active');
            $('.map-block[data-id=' + mapTarget + ']').addClass('active').siblings().removeClass('active');
            mapToolTip.text(mapTarget);
            mapToolTip.text(function () {
                return $(this).text().replace('_', ' ');
            });
        }
    ...
    $('.map-list button').on('click touchstart', function () {
        $(this).addClass('active').parent().siblings().find('.active').removeClass('active');
        $(this).closest('ul').siblings().find('.active').removeClass('active');
        var mapTarget = $(this).attr('data-target');
        var mapTargetElement = $('.map-img path[data-id=' + mapTarget + ']');
        var mapExtra = $(this).attr('data-extra');
        var mapElementTop = mapTargetElement.offset().top;
        var mapElementLeft = mapTargetElement.offset().left;
        mapTargetElement.addClass('active').siblings().removeClass('active');
        $('.map-block[data-id=' + mapTarget + ']').addClass('active').siblings().removeClass('active');
        $('.map-block[data-extra=' + mapExtra + ']').addClass('active').siblings().removeClass('active');
        mapToolTip.css({
            top: mapElementTop + 'px',
            left: mapElementLeft + 'px'
        }).text($(this).text());
    ...
    $('.counselor-button').on('click', function () {
       $(this).closest('.counselor-wrap').toggleClass('open');
    });
